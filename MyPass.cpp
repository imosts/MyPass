...
//
//                            inst++;
//                            LoadInst *sameLI = new LoadInst(LI->getPointerOperand(), "", &(*inst));
//                            BitCastInst *BCI = new BitCastInst(sameLI, PointerType::getUnqual(PointerType::getUnqual(Type::getInt8Ty(tmpF->getContext()))), "loadBCI", &(*inst));
//                            std::vector<Value *> LoadList;
//                            LoadList.push_back(BCI);
//                            ArrayRef<Value *> funcArg(LoadList);
//                            Value *func = tmpF->getParent()->getFunction("Load");
//                            CallInst *nCI = CallInst::Create(func, funcArg, "", &(*inst));
//                            BitCastInst *afterLoad = new BitCastInst(nCI, LI->getType(), "aftBCI", &(*inst));
//                            LI->replaceAllUsesWith(afterLoad);
//
//                            int useNum = afterLoad->getNumUses();
//                            if(useNum >= 1){
...
//                                        if (SI->getValueOperand() == afterLoad) {
//                                            SI->setOperand(0, LI);
//                                            isLIUse = true;
//                                            useNum--;
//                                            continue;
...
//                            if (!isLIUse) {
//                                LI->eraseFromParent();
////                                insertLoadCheckInBasicBlock(tmpF, bb, inst, sameLI);
//
//                                continue;
//
//                            }
//
//                            if (afterLoad->getNumUses() == 0) {
//                                afterLoad->eraseFromParent();
//                                nCI->eraseFromParent();
//                                BCI->eraseFromParent();
//                                sameLI->eraseFromParent();
//                            }
//
//                            inst--;
//
//
//
//                        }else if ((pointerLevel(LI->getOperand(0)->getType()) == 1 && !dyn_cast<AllocaInst>(LI->getOperand(0)))) {
//                            BitCastInst *BCI = new BitCastInst(LI->getOperand(0), PointerType::getUnqual(PointerType::getUnqual(Type::getInt8Ty(tmpF->getContext()))), "loadBCI", &(*inst));
//                            std::vector<Value *> LoadList;
//                            LoadList.push_back(BCI);
//                            ArrayRef<Value *> funcArg(LoadList);
//                            Value *func = tmpF->getParent()->getFunction("Load");
//                            CallInst *nCI = CallInst::Create(func, funcArg, "", &(*inst));
//                            BitCastInst *afterLoad = new BitCastInst(nCI, LI->getOperand(0)->getType(), "aftBCI", &(*inst));
//                            LI->setOperand(0, afterLoad);
//
//                        }
//
//                    }
...
//
//                                                            BCI->setOperand(0, callLoad);
//                                                            CI->setOperand(oi->getOperandNo(), BCI);
//                                                        }
//                                                    }
                                                }
                                            }
                                            
//                                            if (CI->getCalledFunction()->getName().contains("free") && isComeFormSouce(CI->getOperand(0), LI)) {
//                                                LIUseNum++;
//                                                useNum--;
//                                                continue;
//                                            }
...
//                                        else if(BitCastInst *BCI = dyn_cast<BitCastInst>(inst2)) {
//                                            BasicBlock::iterator tmpInst = inst2;
//                                            tmpInst++;
//                                            if (CallInst *CI = dyn_cast<CallInst>(tmpInst)){
//                                                if (CI->getCalledFunction()->getName().contains("free") && CI->getOperand(0) == BCI) {
//                                                    if (BCI->getOperand(0) == LI) {
//                                                        BCI->setOperand(0, LI->getPointerOperand());
//                                                        BCI->mutateType(PointerType::getUnqual(BCI->getType()));
//                                                        continue;
//                                                    }
//                                                }
...
//                        else if ((pointerLevel(LI->getOperand(0)->getType()) == 1)){
//                            LI->setOperand(0, insertLoadCheckInBasicBlock(tmpF, bb, inst, LI->getPointerOperand()));
//                        }
...
//                            BinaryOperator *BO = BinaryOperator::Create(Instruction::BinaryOps::And, PTI, ConstantInt::get(Type::getInt64Ty(bb->getContext()), AND_PTR_VALUE, false), "", &(*inst));
//                            IntToPtrInst *ITP = new IntToPtrInst(BO, SI->getPointerOperand()->getType(), "", &(*inst));
//                            SI->setOperand(1, ITP);
//                        }
                    }
...
//                            if (PHINode *phi = dyn_cast<PHINode>(GEP->getPointerOperand())) {
//                                PtrToIntInst *PTI = new PtrToIntInst(GEP->getPointerOperand(), Type::getInt64Ty(bb->getContext()), "", &(*inst));
//                                BinaryOperator *BO = BinaryOperator::Create(Instruction::BinaryOps::And, PTI, ConstantInt::get(Type::getInt64Ty(bb->getContext()), AND_PTR_VALUE, false), "", &(*inst));
//                                IntToPtrInst *ITPmulti = new IntToPtrInst(BO, GEP->getPointerOperand()->getType(), "", &(*inst));
//                                GEP->setOperand(0, ITPmulti);
//                            }
//                        }
//                    }
                    inst->dump();
